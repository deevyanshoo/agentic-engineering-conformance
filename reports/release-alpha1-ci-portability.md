...
The first PR workflow at head `7afbc58dc30ef9506709e33bb0cbf1b62ea3fd26` (run `33326841914`, job `99298389337`) executed all repository setup, Ruff, and mypy steps, then reported 18 failed and 245 passed tests. The remaining failure was boundary placement: plan writing and injected deterministic runtime seams were incorrectly required to use current-OS host executables. The first attempted correction made compatibility conditional on selection of the default factory; that approach was superseded by the two focused review remediations below. The current worker requires local executable flavour for every factory unless the explicit, pre-invocation-validated non-subprocess test seam is used. Portable plan persistence remains OS-neutral.
...
`bind_plan_to_local_runtime` is required by plan writing, scheduler launch, and worker execution. It always rejects foreign source/output identities before local filesystem use. Plan writing intentionally permits portable foreign executable identities; the worker requests executable-flavour enforcement by default and rejects incompatibility before source I/O, host probe, or model execution. Only the explicit prevalidated non-subprocess test seam disables that executable check. Aggregate/inspection code may still load foreign plans.
