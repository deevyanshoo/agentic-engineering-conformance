...
The real-host evidence is deliberately small. Historical M2/M3 trials are N=1 integration proofs; M4 is N=3 per host; M5's twelve planned paired slots were terminally invalid before any model process due a repaired Windows path defect. These records support architecture and experimental-runner observations only - not rankings, pass rates, statistical superiority, security certification, or global host conformance. See [claims](docs/claims.md) and [public evidence policy](docs/evidence-policy.md).
